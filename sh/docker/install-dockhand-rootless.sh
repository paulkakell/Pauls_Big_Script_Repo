...
DOCKER_UID="$(ask "User UID" "100000")"
DOCKER_GID="$(ask "User GID" "100000")"
...
apt-get update
apt-get install -y ca-certificates curl gnupg uidmap dbus-user-session slirp4netns fuse-overlayfs openssl
...
chown -R "$DOCKER_UID:$DOCKER_GID" "$HOST_PATH"
...
chown -R "$DOCKER_UID:$DOCKER_GID" "$INSTALL_PATH"
